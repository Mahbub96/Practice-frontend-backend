...
function show(player) {
	...
	output.innerHTML = "<span style='background-color:lightgreen;'>" + player[0].name + player[0].value + "<br> " + player[1].name + player[1].value + " <br>" + player[2].name + player[2].value + "<br>" + player[3].name + player[3].value + "</span>";
...
function addElement(player) {
	...
	//    check if inputed value is not zero 
	const newContent0 = ifZeroFound(f1);
	const newContent1 = ifZeroFound(f2);
	const newContent2 = ifZeroFound(f3);
	const newContent3 = ifZeroFound(f4);
...
// add red and green signal 
function ifZeroFound(a) {
	if (a.value == 0) return addElementAt("mark");
	else return document.createTextNode(parseInt(a.value));

}
function addElementAt(name) {

	const newDiv = document.createElement(name);
	let v = document.createTextNode(0);
